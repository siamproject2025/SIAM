...
// DELETE /api/grados/:id (soft)
exports.eliminarGrado = async (req, res) => {
  try {
    const { id } = req.params;
    if (!isValidObjectId(id)) return res.status(400).json({ message: "ID inválido." });

    const desactivado = await Grado.findByIdAndUpdate(id, { estado: "Inactivo" }, { new: true });
    if (!desactivado) return res.status(404).json({ message: "Grado no encontrado" });
    return res.json({ message: "Grado desactivado", data: desactivado });
  } catch (err) {
    return res.status(500).json({ message: "Error al desactivar grado", error: err.message });
  }
};
