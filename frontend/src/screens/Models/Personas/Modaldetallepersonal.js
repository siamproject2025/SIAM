      ...
      <motion.div className="per-modal lg" initial={{ opacity:0, y:-20, scale:0.96 }} animate={{ opacity:1, y:0, scale:1 }} transition={{ duration:0.25 }}>
        ...
        <div className="per-modal-footer">
          ...
          <div style={{ display:"flex", gap:".75rem" }}>
            <button style={S.btn('#E74C3C')} onClick={() => {
            if (window.confirm(`¿Eliminar al empleado "${empleado.nombres} ${empleado.apellidos}"?`)) onDelete(empleado._id);
          }}>
            Eliminar
          </button>
            <button  style={S.btn('#E0D9F5','#6C4FBF')} onClick={onClose}>Cancelar</button>
            <button style={S.btn('#6C4FBF')} onClick={handleSubmit}>Guardar</button>
          </div>
        </div>
      </motion.div>
    </div>
