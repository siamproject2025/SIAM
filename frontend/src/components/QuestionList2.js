...
const QuestionList = () => {
    ...
    // ⭐ Solo se permite preguntar (true), NO responder (false)
    const canAsk = true; 
    const canAnswer = false; // DESHABILITADO

    return (
        <div className="question-list-container">
            <h2>Módulo de Consultas (Solo Preguntar y Ver Respuestas)</h2>
                ...
                {questions.length === 0 && !error ? (
                    ...
                    questions.map((q) => (
                        <QuestionItem 
                            key={q._id} 
                            question={q} 
                            canAnswer={canAnswer} // Pasa 'false' para deshabilitar el formulario
